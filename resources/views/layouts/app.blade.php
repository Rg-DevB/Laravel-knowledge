{{-- ============================================================
KNOWRAVEL — resources/views/layouts/app.blade.php
...
<head>
    ...
    <!-- <title>{{ $title ?? 'Knowravel' }} — Laravel Problem Database</title> -->
    <title>{{ $title ?? config('app.name') }}</title>

    <meta name="description"
        content="The collaborative knowledge base for Laravel developers. Find solutions, share fixes, learn from the community.">
        ...
        <aside class="hidden lg:flex flex-col w-60 border-r border-zinc-800/60 bg-[#0d0d14] fixed h-full z-30">
            ...
            <div class="px-5 py-4 border-b border-zinc-800/60">
                <a href="{{ route('home') }}" wire:navigate class="flex items-center gap-2.5">
                    <div class="w-7 h-7 rounded-lg text-rose-500 flex items-center justify-center">
                        <x-app-logo-icon class="w-7 h-7 fill-current" />
                    </div>
                    <span class="font-semibold text-sm text-zinc-100">Knowravel</span>
                </a>
            </div>
            ...
            <div class="border-t border-zinc-800/60 p-3">
                ...
                @else
                    <div class="flex gap-2">
                        <a href="{{ route('login') }}"
                            class="flex-1 text-center text-xs py-1.5 rounded-md border border-zinc-700 text-zinc-400 hover:text-zinc-200 hover:border-zinc-600 transition-all">Sign
                            in</a>
                        <a href="{{ route('register') }}"
                            class="flex-1 text-center text-xs py-1.5 rounded-md bg-rose-500/10 border border-rose-500/30 text-rose-400 hover:bg-rose-500/20 transition-all">Register</a>
                    </div>
                @endauth
            </div>
        </aside>
        ...
        <div class="flex-1 lg:ml-60 flex flex-col min-h-screen">
            ...
            <header class="sticky top-0 z-20 border-b border-zinc-800/60 bg-[#0a0a0f]/80 backdrop-blur-xl">
                <div class="flex items-center gap-4 px-4 lg:px-6 h-14">
                    ...
                    <div class="hidden md:flex absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-lg z-10">
                        <livewire:search.search-bar />
                    </div>

                    {{-- Right actions --}}
                    <div class="flex items-center gap-3 ml-auto relative z-20">
                        @auth
                            {{-- Notifications --}}
            ...
            @if(session()->has('success'))
                <div x-data="{ show: true }" x-show="show" x-init="setTimeout(() => show = false, 4000)"
                    ...
                    <svg class="w-4 h-4 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke-width="2"
                        stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round"
            ...
            <footer class="border-t border-zinc-800/60 px-4 lg:px-6 py-4">
                <div class="flex flex-col sm:flex-row items-center justify-between gap-2 text-xs text-zinc-600">
                    <span>Knowravel — Built with ❤️ and Laravel</span>
