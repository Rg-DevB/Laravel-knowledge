<div class="space-y-3">
    ...
    <div class="flex gap-3" x-data="{ showReply: false }">
        <img src="{{ $comment->user->avatar ?? 'https://ui-avatars.com/api/?name='.urlencode($comment->user->name).'&background=1e1e2e&color=a78bfa&size=32' }}"
             class="w-7 h-7 rounded-full shrink-0" alt="{{ htmlspecialchars($comment->user->name, ENT_QUOTES, 'UTF-8') }}">
        <div class="flex-1">
            <div class="flex items-center gap-2 mb-1">
                <span class="text-xs font-medium text-zinc-300">{{ htmlspecialchars($comment->user->name, ENT_QUOTES, 'UTF-8') }}</span>
                <span class="text-xs text-zinc-600">{{ $comment->created_at->diffForHumans() }}</span>
            </div>
            <p class="text-sm text-zinc-400">{{ htmlspecialchars($comment->content, ENT_QUOTES, 'UTF-8') }}</p>
            ...
            <div class="mt-3 pl-4 border-l border-zinc-800 space-y-3">
                ...
                <div class="flex gap-2">
                    <img src="{{ $reply->user->avatar ?? 'https://ui-avatars.com/api/?name='.urlencode($reply->user->name).'&background=1e1e2e&color=a78bfa&size=24' }}"
                         class="w-5 h-5 rounded-full shrink-0" alt="{{ htmlspecialchars($reply->user->name, ENT_QUOTES, 'UTF-8') }}">
                    <div>
                        <div class="flex items-center gap-2">
                            <span class="text-xs font-medium text-zinc-400">{{ htmlspecialchars($reply->user->name, ENT_QUOTES, 'UTF-8') }}</span>
                            <span class="text-xs text-zinc-600">{{ $reply->created_at->diffForHumans() }}</span>
                        </div>
                        <p class="text-xs text-zinc-500 mt-0.5">{{ htmlspecialchars($reply->content, ENT_QUOTES, 'UTF-8') }}</p>
                    </div>
                </div>
                @endforeach
            </div>
